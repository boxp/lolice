...
- 通常ウィンドウ: 開始まで20〜45分のGoogleカレンダーイベントを検索（25分幅で15分間隔cronの境界ずれ・実行遅延時の取りこぼしを防ぐ; 重複はID永続化で抑止）
- 09:00-09:14の実行時: 静音時間（02:00-09:00）中に通知できなかった直近25分前〜20分未満の予定を補足通知（通常ウィンドウと重複しない）
- 対象イベントがあれば「📅 N分後に〜が始まります」（実際の分数を表示; 補足時は「まもなく（静音中に通知できなかった予定）」）と出力
- 対象なし → 完全沈黙（何も出力しない）
