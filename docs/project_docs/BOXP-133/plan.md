...
- 通常ウィンドウ: 開始まで25〜40分のGoogleカレンダーイベントを検索（15分幅でcronの15分間隔と一致させ取りこぼしを防ぐ）
- 09:00-09:14の実行時: 静音時間（02:00-09:00）中に通知できなかった直近1〜25分以内の予定を補足通知
- 対象イベントがあれば「📅 30分後に〜が始まります」（補足時は「まもなく（静音中に通知できなかった予定）」）と出力
- 対象なし → 完全沈黙（何も出力しない）
