...
- 2026-07-05: `kubectl kustomize argoproj/codex-workspace` succeeded.
- 2026-07-05: ConfigMap から展開した `server.py` は `python3 -m py_compile` succeeded.
- 2026-07-05: 空の `runs/` fixture で `/api/runs` が `runs: []` を返すことを確認した。
- 2026-07-05: 完了済み run / ログ欠損 fixture で `/api/runs` が `finished` を返し、log API が `missing: true` を返すことを確認した。
- 2026-07-05: lock file あり fixture で `/api/runs` が `running` を返し、`events.jsonl` を offset 付きで読めることを確認した。
- 2026-07-05: `/app.js` は `node --check` succeeded。`POST /api/runs` は 405 を返すことを確認した。
