...
- The Argo CD kustomize image override resolves every codex-workspace container to the verified, published `sha-5f0ba57` image built from the companion PR head and containing `novel_board_runner.bb`.
...
The companion `boxp/arch` PR #11014 workflow dispatch published `sha-5f0ba57` from head commit `5f0ba5756490200bc55061d0225ab68c6f3eaff4`. This manifest pins that verified image in `.argocd-source-codex-workspace.yaml`; the publishing run is [Build Codex Workspace Image #29143364892](https://github.com/boxp/arch/actions/runs/29143364892). A later image-updater commit may advance the tag only to another image that also contains `novel_board_runner.bb`.
