...
この変更は companion の `boxp/arch` PR #11014 の head commit `5f0ba5756490200bc55061d0225ab68c6f3eaff4` を workflow dispatch で公開した `sha-5f0ba57` image を `.argocd-source-codex-workspace.yaml` に固定しています。[publishing run](https://github.com/boxp/arch/actions/runs/29143364892) の成功と、image に `/opt/codex-workspace/novel-board/novel_board_runner.bb` と Novel Board seed が含まれることを確認済みです。image updater による別 tag への更新だけを前提にしないため、manifest を先行させても sidecar の command path が欠けた image は選ばれません。
...
active Novel run がないことを確認して、この `novel-board-runner` sidecar 追加だけを先に revert します。private root と管理ノート、完成版は削除しません。`boxp/arch` image の runner/seed は sidecar がなければ実行されないため、image revert は必須ではありません。再導入時は current lane と既存 `published.edn` から冪等に再開します。
