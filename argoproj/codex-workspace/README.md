...
この変更は companion の `boxp/arch` PR #11014 の head commit `82edb0c45f9dd9353a509c257ecbaa0db9f493c4` を workflow dispatch で公開した `sha-82edb0c` image を `.argocd-source-codex-workspace.yaml` に固定しています。[publishing run](https://github.com/boxp/arch/actions/runs/29147800166) の成功、registry digest `sha256:9c83a9e2a34492b8c6afd1df4c4e3d1b9688a295ebc9691f1b7aa0bae783b83d`、image 内の Novel runner / seed / Pi vision input / root・UID 1000 の両方に対応する `CODEX_WORKSPACE_ROLE=novel-board-runner` entrypoint 契約を確認済みです。image updater による別 tag への更新だけを前提にしないため、manifest を先行させても runner role を起動できない image は選ばれません。
