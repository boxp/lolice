...
この変更は companion の `boxp/arch` PR #11014 の head commit `1d32a00709fd68decbe5713014b2fa3ac5d2e29e` を workflow dispatch で公開した `sha-1d32a00` image を `.argocd-source-codex-workspace.yaml` に固定しています。[publishing run](https://github.com/boxp/arch/actions/runs/29149479751) の成功、registry digest `sha256:87a02852d13d79802bb8a1a2df40f113a4d09149a1a7033b9d79d207d6e559cb`、image 内の Novel runner / title-only scaffold / Board・template・README seed / Pi vision input / process-shared note update lock / root・UID 1000 の両方に対応する `CODEX_WORKSPACE_ROLE=novel-board-runner` entrypoint 契約を確認済みです。image updater による別 tag への更新だけを前提にしないため、manifest を先行させても runner role を起動できない image は選ばれません。
