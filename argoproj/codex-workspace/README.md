...
この変更は companion の `boxp/arch` PR #11014 の head commit `6fa3915bade8482e6c51eaf911e92d8266ed84bf` を workflow dispatch で公開した `sha-6fa3915` image を `.argocd-source-codex-workspace.yaml` に固定しています。[publishing run](https://github.com/boxp/arch/actions/runs/29143955512) の成功、registry digest `sha256:9013eaff919b24c14e88636c28fdd27d432963eeb685836666c060255e7dd053`、image 内の Novel runner / seed / `CODEX_WORKSPACE_ROLE=novel-board-runner` entrypoint 契約を確認済みです。image updater による別 tag への更新だけを前提にしないため、manifest を先行させても runner role を起動できない image は選ばれません。
