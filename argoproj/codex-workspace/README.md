...
この変更は companion の `boxp/arch` PR #11014 の最新 head を含む CI merge commit から build された `sha-3759085` image を `.argocd-source-codex-workspace.yaml` に固定しています。この image の build-and-push が成功し、`/opt/codex-workspace/novel-board/novel_board_runner.bb` と Novel Board seed を含むことを確認してから manifest を merge / sync します。image updater による別 tag への更新だけを前提にしないため、manifest を先行させても sidecar の command path が欠けた image は選ばれません。
