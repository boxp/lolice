...
companion の `boxp/arch` PR #11014 は merge commit `bf7fbd0085659e5cb93996e9fa7d7f87db787db5` として取り込み済みです。現在 `.argocd-source-codex-workspace.yaml` が選択する `sha-0ff5eb9` はその直後の main commit から [Build Codex Workspace Image #29152066494](https://github.com/boxp/arch/actions/runs/29152066494) で公開され、amd64 manifest digest `sha256:d06aa4c92246b095d9f5dcccc9aa5136b376eee433c9704072e2d24afe679180` を確認済みです。この image には Novel runner / title-only scaffold / レーン別 `#novel-rule` と管理ノート Workflow seed / Pi vision input / process-shared note update lock / publish lock heartbeat / root・UID 1000 の両方に対応する `CODEX_WORKSPACE_ROLE=novel-board-runner` entrypoint 契約が含まれます。今後 image updater が tag を進める場合も、PR #11014 の merge commit を祖先に持ち、この契約を含む image だけを使用します。
