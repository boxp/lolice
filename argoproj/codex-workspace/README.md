...
companion の `boxp/arch` PR を先に merge し、`/opt/codex-workspace/novel-board/novel_board_runner.bb` と Novel Board seed を含む image が生成されたことを確認します。その image を `.argocd-source-codex-workspace.yaml` が参照した後に、この manifest を merge / sync します。manifest を先行させると sidecar の command path が存在せず Pod が Ready にならないため、順序を逆にしません。
...
kubectl -n "${NS}" exec "${POD}" -c novel-board-runner -- sh -c \
  'test -f "$CODEX_NOVEL_BOARD_VAULT/Boards/Novel Board.md" && test -d "$CODEX_NOVEL_BOARD_ROOT"'
```
